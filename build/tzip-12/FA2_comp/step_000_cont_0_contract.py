...
class Contract(sp.Contract):
  def __init__(self):
    ...
    self.init(administrator = sp.address('tz1g6QGhMSB89TswxYwPsZgAMUoikWAXQEtW'),
              all_tokens = 0,
              ledger = {},
  ...
  def balance_of(self, params):
    ...
    def f_x0(_x0):
      sp.verify(self.data.token_metadata.contains(f_x0.token_id), 'FA2_TOKEN_UNDEFINED')
      sp.if self.data.ledger.contains((sp.set_type_expr(f_x0.owner, sp.TAddress), sp.set_type_expr(f_x0.token_id, sp.TNat))):
        sp.result(sp.record(request = sp.record(owner = sp.set_type_expr(f_x0.owner, sp.TAddress), token_id = sp.set_type_expr(f_x0.token_id, sp.TNat)), balance = self.data.ledger[(sp.set_type_expr(f_x0.owner, sp.TAddress), sp.set_type_expr(f_x0.token_id, sp.TNat))].balance))
      sp.else:
        sp.result(sp.record(request = sp.record(owner = sp.set_type_expr(f_x0.owner, sp.TAddress), token_id = sp.set_type_expr(f_x0.token_id, sp.TNat)), balance = 0))
    responses = sp.local("responses", params.requests.map(sp.build_lambda(f__x0)))
